...
class RoomList:
    ...
    def delete_room(self, room_id):
        """Delete a room if it hasn't had users, or archive it if it has"""
        ...
        if room.has_had_participants:
            room.archive()
            self.archived_rooms[room_id] = room
    ...
    def get(self, room_id, default=None, include_archived=False):
        return (
            self.rooms.get(room_id)
            or (include_archived and self.archived_rooms.get(room_id))
            or default
        )
