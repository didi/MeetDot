...
class Room:
    ...
    def add_participant(self, participant):
        ...
        if self.is_captioning_active and not participant.is_audience:
            self.manager.add_participant(user_id, participant)
    ...
    def caption_languages(self) -> set:
        return set(sum((p.caption_languages for p in self.participants.values()), []))
    ...
    def archive(self):
        """
        Prepare self for archival by removing things that are large in memory
        """
        self.manager.speech_translator = None
