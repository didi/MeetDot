...
# other repo modules
from namespaces.feedback import FeedbackNamespace
from namespaces.meeting import MeetingNamespace
from namespaces.word_guessing import WordGuessingNamespace
from room import (
    AudienceMember,
    Participant,
    Room,
...
class SpeechTranslationServer:
    def __init__(self, translation_config_file, log_dir=Path("logs")):
        ...
        # Set up namespaces
        self.namespaces = {
            "meeting": MeetingNamespace(self.rooms, self.socketio.emit),
            "word_game": WordGuessingNamespace(self.rooms, self.socketio.emit),
            "feedback": FeedbackNamespace(self.rooms, self.socketio.emit),
        }
    ...
    def setup_rest_endpoints(self):
        ...
        @self.app.route("/full_transcript/<room_id>/<lang>", methods=["GET"])
        def full_transcript(room_id, lang):
            room = self.rooms.get(room_id)
            transcript = room.manager.prepare_transcript(lang)
            return Response(
                transcript,
                mimetype="text/plain",
                headers={
                    "Content-disposition": f"attachment;filename=transcript-{lang}.txt"
                },
            )
    ...
    def setup_socket_endpoints(self):
        ...
        def on_joined_room(msg):
            ...
            if is_audience:
                participant = AudienceMember(msg["captionLanguage"])
            else:
                participant = Participant(
                    msg["name"], msg["spokenLanguage"], msg["captionLanguage"]
                )

            room.add_participant(msg["userId"], participant)

            self.rooms.room_changed(room)

            if room.is_captioning_active:
                history = room.manager.translations.get(msg["captionLanguage"], [])
        ...
        def on_caption_language_changed(msg):
            ...
            room.manager.add_new_language(language)
            participant.caption_language = language
            self.rooms.room_changed(room)
    ...
    def get_certificates(self):
        ...
        if int(os.getenv("HTTPS_ENABLED", 0)):
            ...
            if (ssl_path / "cert.pem").exists() and (ssl_path / "privkey.pem").exists():
                context = ssl.create_default_context(ssl.Purpose.CLIENT_AUTH)
                context.load_cert_chain(
                    certfile=ssl_path / "cert.pem", keyfile=ssl_path / "privkey.pem"
                )
