...
This uses a producer-consumer setup, where the Translator produces data, and the
TranslationEventLoop consumes it. The event loop throttles the translation service
to only run once every min_interval_ms milliseconds.
"""
import threading
import time
...
class TranslationEventLoop:
    ...
    def set_request(self, request, callback):
    ...
    def loop(self):
        while True:
            ...
            if request:
                ...
                self.translation_fn(request, self.callback_fn)
...
class Translator:
    ...
    def __call__(self, request: TranslationRequest, callback: Callable):
        ...
        with event_loop.condition:
            event_loop.set_request(request, callback)

            if request.is_final:
                event_loop.latest_completed_request_text = ""
            event_loop.condition.notify()

    def call_translate(self, request, callback: Callable):
