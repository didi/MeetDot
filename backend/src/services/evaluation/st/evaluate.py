...
def on_caption(
    ...
    input_translation = combine_utterances(
        request.language, prev_utterances, request.utterance, request.utterance_complete
    )

    word_separator = get_word_separator(request.language)
...
def evaluate_dataset(
    ...
    if all(f.exists() for f in output_files) and not overwrite:
        ...
        speech_translator = SpeechTranslationService(
            config=config,
            logger=mock_logger,
            languages=lambda: (dataset.target_language,),
        )
