...
class SpeechTranslationDataset:
    ...
    def load_dataset(self, dataset_name: str):
        ...
        elif dataset_name.startswith("multi_ted") and dataset_name.count("-") == 2:
            self.load_multi_ted_dataset(dataset_name)
        else:
    ...
    def load_multi_ted_dataset(self, dataset_name: str):
        dataset_basename, source_language, target_language = dataset_name.split("-")
        self.source_language = SpeechTranslationDataset.LANGUAGE_CODES.get(
            source_language, source_language
        )
        ...
        dataset_dir = (
            Path(self.dataset_dir)
            / f"{source_language}-{target_language}/data/iwslt2021"
        )

        transcript_filepath = str(dataset_dir / f"txt/iwslt2021.{source_language}")
        translation_filepath = str(dataset_dir / f"txt/iwslt2021.{target_language}")
        segments_filepath = str(dataset_dir / "txt/segments")
        yaml_filepath = str(dataset_dir / "txt/iwslt2021.yaml")

        self.wav_files, self.transcripts, self.translations = [], [], []
        self.transcripts = load_text_file(transcript_filepath)
        self.translations = load_text_file(translation_filepath)

        with open(yaml_filepath, "r") as file_info:
            file_yaml = yaml.load(file_info, Loader=yaml.FullLoader)
        if file_yaml is not None:
            with open(segments_filepath, "r") as segments:
                for l in file_yaml:
                    seg_line = segments.readline()
                    file_id = seg_line.split()[0]
                    wav_file = file_id
                    file_path = str(dataset_dir / f"wav_processed/{wav_file}.wav")
                    self.wav_files.append((file_id, file_path))
                    self.names.append("")
                    self.speakers.append(l["speaker_id"])

        self.logger.info(f"Loaded {len(self.transcripts)} utterances from multi-ted")
