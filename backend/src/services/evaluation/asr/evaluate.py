...
from .language_id import parse_language_id_block, generate_images
from .score import (
...
def evaluate_dataset(
    ...
    language_id_visualization_path = (
        output_dir / f"{dataset.name}_language_id_visualization"
    )
    ...
    if (
        type(dataset.language) is dict
    ):  # this means we have a language id reference file for this dataset.
        reference_durations = {
        ...
        logger.info(
            f"Language ID visualization saved to {language_id_visualization_path}"
        )

        logger.info(f"Other Language ID metrics on {dataset.name} for each file:")
        for file_id in dataset.language:
            gold_df = pd.DataFrame(dataset.language[file_id])
            sys_df = pd.DataFrame(detected_languages[file_id])
            lid_scores = LanguageIDEvaluate(gold_df, sys_df)
            (
                language_id_precision,
                language_id_recall,
                language_id_latency,
            ) = lid_scores()
            language_id_corruption = 1 - score_language_id_accuracy(
                {file_id: dataset.language[file_id]},
                {file_id: reference_durations[file_id]},
                {file_id: detected_languages[file_id]},
            )
            logger.info(file_id)
            logger.info(f"precision: {language_id_precision}")
            logger.info(f"recall: {language_id_recall}")
            logger.info(f"latency: {language_id_latency}")
            logger.info(f"corruption: {language_id_corruption}")
