...
class LinewiseScrollStrategy(CaptionStrategy):
    ...
    def __call__(self, request: CaptioningRequest) -> Tuple[CaptioningResponse, float]:
        delay_time = 0.0
        key = (request.session_id, request.original_language, request.target_language)
        ...
        if not request.utterance_complete:
            ...
            highlight_boundaries = LinewiseScrollStrategy.get_highlight(
                self.prev_lines[key][line_index - self.prev_line_index[key] :],
                lines,
                line_index,
            )
        else:
        ...
        if not self.config.enable_highlight:
            highlight_boundaries = None

        self.prev_lines[key] = lines
        self.prev_line_index[key] = line_index
