...
class PostTranslationService:
    ...
    def post_translation(
        ...
        # Add ". " or "。" to the end of sentences if needed
        if asr_is_final and not is_chat:
            translation = translation.strip()
            if translation and translation[-1] in ".!?｡。！？":
                delimiter = languages[language].word_separator
            else:
                delimiter = (
                    languages[language].eos_punctuation
                    + languages[language].word_separator
                )
    ...
    def _update_translate_k(self, key, asr_is_final):
        ...
        return asr_is_final or ct % self.config.translate_k == 0
