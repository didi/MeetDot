...
# Chinese text normalization code, from https://github.com/speechio/chinese_text_normalization
...
def chn2num(chinese_string, numbering_type=NUMBERING_TYPES[1]):
    ...
    def compute_value(integer_symbols):
        """
        Compute the value.
        When current unit is larger than previous unit, current unit * all previous units
        will be used as all previous units.
...
if __name__ == "__main__":
    ...
    for l in ifile:
        ...
        old_chars = (
            CHINESE_PUNC_LIST + string.punctuation
        ...
        if args.has_key:
            ...
            if (
                text.strip() != ""
            ):  # skip empty line in pure text format(without Kaldi's utt key)
                ofile.write(text + "\n")
