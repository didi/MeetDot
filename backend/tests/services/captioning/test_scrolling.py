...
@pytest.mark.parametrize(
    "num_lines,s,language,chars_per_line,expected", test_punctuation_sensitive_data
)
...
@pytest.mark.parametrize(
    "prev_lines, text, language, expected, expected_boundaries", test_highlight_data
)
def test_highlight(
    prev_lines: list,
    text: str,
    ...
    session_id = "session"
    strategy = LinewiseScrollStrategy(config)
    strategy.prev_lines[("session", language)] = prev_lines
    request = CaptioningRequest(
...
def test_multiple_speakers():
    ...
    strategy.prev_lines[("speaker1", "en-US")] = ["Hello"]
    strategy.prev_lines[("speaker2", "es-ES")] = ["Hola"]
