  ...
  <div id="transcription-page" v-show="roomExistChecked">
    ...
    <div class="settings">
      ...
      <div class="setting captions">
        <p v-t="'Live.caption_language'" />
        <select v-model="captionLanguage">
          <option
            v-for="(val, key) in $globals.endonyms"
            :key="key"
            :value="key"
...
export default {
  ...
  data() {
    return {
      ...
      stream: null,
      room: null,
      captionMode: CAPTION_MODES.MONOLINGUAL,
    };
  },
  mounted() {
    ...
    backendService()
      .post(
        ...
        JSON.stringify({
          ...
          settings: {
            services: {
              asr: { provider: "kaldi", chunk_size: 1600 },
      ...
      .then(() => {
        ...
        return backendService().get("/rooms/" + roomId);
      })
      .then((response) => {
  ...
  methods: {
    ...
    onJoined(couldJoin, errorMessage) {
      if (!couldJoin) {
        ...
        this.sockets.subscribe(`/${this.captionLanguage}/translation`, (msg) =>
    ...
    gotDevices(deviceInfos) {
      ...
      const interval = setInterval(() => {
        const loadedMicrophoneNames = this.audioInputs[0].label.length;
        if (loadedMicrophoneNames) {
          clearInterval(interval);
        }
      }, 1000);
    },
    onTranslation(msg) {
      if (this.translationLines === undefined) {
      ...
      if (lineAdded) {
        ...
        this.$nextTick(() => {
          // this.$refs.captions.lastElementChild.scrollIntoView({ behavior: "smooth" });
        });
      }
      this.$forceUpdate();
    },
  ...
  watch: {
    captionLanguage(newCaptionLang, oldCaptionLang) {
      this.$socket.emit(
        "/caption-language-changed",
        {
          userId: this.userId,
          roomId: this.room.room_id,
          languages: [newCaptionLang],
        },
        (success) => {
          if (!success) {
            Vue.$toast.error(this.$t("Room.error_could_not_change_language"));
          } else {
            this.sockets.unsubscribe(`/${oldCaptionLang}/translation`);
            this.sockets.subscribe(
              `/${newCaptionLang}/translation`,
              this.onTranslation
            );
...
.captions {
  ...
  .language-block {
    display: block;
    height: calc(100% / var(--num-languages));
    padding-bottom: 20px;
  }
}
.settings {
