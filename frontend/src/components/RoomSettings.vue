<template>
  <div>
    ...
    <transition-group
      tag="div"
      name="smooth-list"
      v-if="defaultSettingsLoaded && lang2asrLoaded"
    >
      ...
      >
        ...
        <div class="setting-group">
          <div v-for="(asrs, lang) in lang2asr" :key="lang">
            <!-- TODO(chris): How to use [v-t="'Settings.asr_system'"] here -->
            <h4>ASR {{ lang }}</h4>
            <div v-for="asr in asrs" :key="asr" class="inline-block">
              <label :for="asr">{{ asr }}</label>
              <input
                type="radio"
                :id="asr"
                :value="asr"
                v-model="settings.st_services.asr.providers[lang]"
              />
            </div>
          </div>
        </div>
        {{ $t("Settings.stability_threshold") }}
        {{ settings.st_services.asr.stability_threshold }}
        <div class="slider">
          <vue-slider
            v-model="settings.st_services.asr.stability_threshold"
            :min="0"
            :max="1"
            :interval="0.01"
          />
        ...
        <div class="checkbox">
          <input
            type="checkbox"
            id="language-detect"
            v-model="settings.st_services.asr.language_id.enabled"
          />
          <label
            for="language-detect"
        ...
        <div>
          <label v-t="'Settings.language_id_num_consecutive_threshold'"></label>
          <div class="slider">
            <vue-slider
              v-model="
                settings.st_services.asr.language_id.num_consecutive_threshold
              "
              :min="3"
              :max="8"
              :interval="1"
        ...
        <div>
          <label v-t="'Settings.language_id_window_size_seconds'"></label>
          <div class="slider">
            <vue-slider
              v-model="settings.st_services.asr.language_id.window_size_seconds"
              :min="3"
              :max="10"
              :interval="1"
              :adsorb="true"
        ...
        <div>
          <label v-t="'Settings.language_id_window_stride_seconds'"></label>
          <div class="slider">
            <vue-slider
              v-model="
                settings.st_services.asr.language_id.window_stride_seconds
              "
              :min="1"
              :max="3"
              :interval="1"
      ...
      >
        ...
        <div class="setting-group">
          ...
          <input
            ...
            id="google-mt"
            value="google"
            v-model="settings.st_services.translation.provider"
          />
        ...
        <div
          class="setting-group"
          v-if="settings.st_services.translation.provider == 'didi'"
        >
          <p>
            {{ $t("Settings.translation_bias_beta") }}
            {{ settings.st_services.translation.bias_beta }}
          </p>
          <div class="slider">
            <vue-slider
              v-model="settings.st_services.translation.bias_beta"
              :min="0"
              :max="1"
              :interval="0.1"
            />
        ...
        {{ settings.st_services.translation.min_interval_ms }}
        <div class="slider">
          <vue-slider
            v-model="settings.st_services.translation.min_interval_ms"
            :min="0"
            :max="2000"
            :interval="100"
          />
      ...
      >
        ...
        <div
          class="slider"
          v-if="settings.st_services.captioning.strategy !== 'skype_style'"
        >
          <label for="mask-k">Mask-K {{ $t("Settings.mask_k_help") }}</label>
          <div class="slider">
            <vue-slider
              v-model="settings.st_services.post_translation.mask_k"
              :min="0"
              :max="6"
              :interval="1"
              :adsorb="true"
        ...
        <div class="checkbox">
          <input
            ...
            v-model="
              settings.st_services.post_translation.disable_masking_before_k
            "
          />
          <label
        ...
        >
          ...
          <div class="slider">
            <vue-slider
              v-model="settings.st_services.post_translation.translate_k"
              :min="0"
              :max="6"
              :interval="1"
              :adsorb="true"
        ...
        <div class="checkbox">
          <input
            type="checkbox"
            id="remove-profanity"
            v-model="settings.st_services.post_translation.remove_profanity"
          />
        ...
        <div class="checkbox">
          <input
            type="checkbox"
            id="add-punctuation"
            v-model="settings.st_services.post_translation.add_punctuation"
          />
      ...
      >
        ...
        <div class="slider">
          <vue-slider
            v-model="settings.st_services.captioning.num_lines"
            :min="1"
            :max="4"
            :interval="1"
            :adsorb="true"
        ...
        <input
          ...
          v-model="settings.st_services.captioning.punctuation_sensitive"
        />
        <label for="ps" v-t="'Settings.punc_sensitive'"></label>
      </div>
      <div
        class="settings-section"
        v-if="settings.interface.captioning_active"
...
export default {
  ...
  mounted() {
    ...
    backendService()("/lang2asr").then((response) => {
      this.lang2asr = response.data;
      this.lang2asrLoaded = true;
    });
  },
};
</script>
